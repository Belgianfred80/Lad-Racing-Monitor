...
// Vérifie que chaque page répond, affiche son contenu et reste rapide.
// Pendant les heures silencieuses (config quietHours, heure belge),
// seule la disponibilité du site est vérifiée — pas le contenu.
//
...
// Renvoie l'heure belge actuelle en minutes depuis minuit (gère été/hiver tout seul)
function minutesBruxelles() {
...
function enHeuresSilencieuses() {
  if (!config.quietHours) return false;
  const toMin = (s) => {
    const [h, m] = s.split(":").map(Number);
    return h * 60 + m;
  };
  const now = minutesBruxelles();
  const start = toMin(config.quietHours.start);
  const end = toMin(config.quietHours.end);
  if (start <= end) return now >= start && now < end;
  return now >= start || now < end; // fenêtre à cheval sur minuit
}
...
async function checkPage(browser, pageConfig, modeSilencieux) {
  ...
  try {
    ...
    if (modeSilencieux) {
      // Heures silencieuses : le site répond, c'est tout ce qu'on exige.
      result.loadTimeMs = Date.now() - start;
      result.ok = true;
      result.details.push("Verification allegee (heures silencieuses) : le site repond");
    } else {
      ...
      while (Date.now() < deadline) {
        ...
        found = pageConfig.contentSignals.filter((sig) => new RegExp(sig.value).test(bodyText));
...
(async () => {
  const silencieux = enHeuresSilencieuses();
  if (silencieux) {
    console.log("Heures silencieuses (00h00-08h30 heure belge) : verification allegee — disponibilite du site uniquement.");
  }
  const browser = await chromium.launch();
  const results = [];
  for (const pageConfig of config.pages) {
    console.log(`\n▶ Vérification: ${pageConfig.name}`);
    const r = await checkPage(browser, pageConfig, silencieux);
    results.push(r);
